...
def makedir(dest_dir):
    ...
    if not isExist:
        ...
        print("Created directory: " + LIGHT_CYAN + dest_dir + CEND + ".")
...
def tshoot(workdir):
    ...
    delimiter = "******** show "
    # The line count is useful so we can keep track where the information is
    line_count = 0
    last_line = 0
    ...
    while True:
        command = input("Enter a " + LIGHT_CYAN + "[specific command]" + CEND + ", " + LIGHT_CYAN + "[all]" + CEND + " to see all available commands or " + LIGHT_CYAN + "[1]" + CEND + " to quit to main menu: ")

        if command == "1":
            break

        if command == "all":
            print('Available commands: ' + LIGHT_CYAN + ', '.join(str(key) for key, value in commands_dict.items())+ CEND)
...
if __name__ == '__main__':
    try:
        ...
        while True:
            ...
            print("Select " + LIGHT_CYAN + "[3]" + CEND + " to exit.")
            print("------------------------------------------------------------------------")
